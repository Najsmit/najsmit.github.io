<?php
	...
	if($vw_one_page_first_color != false){
		$custom_css .='a, #footer h3, .post-main-box:hover h3 a, .post-navigation a:hover .post-title, .post-navigation a:focus .post-title, #header .main-navigation a:hover, #header .current-menu-item, .woocommerce-message::before, .post-main-box:hover h3 a, .entry-content a, .sidebar .textwidget p a, .textwidget p a, #comments p a, .slider .inner_carousel p a{';
			$custom_css .='color: '.esc_html($vw_one_page_first_color).';';
		$custom_css .='}';
	}
	if($vw_one_page_first_color != false){
		$custom_css .='.logo:after, .catgory-box:hover:after, #about-us hr, .post-info hr, .woocommerce-message, .main-navigation ul ul{';
			$custom_css .='border-top-color: '.esc_html($vw_one_page_first_color).';';
		$custom_css .='}';
	}
	if($vw_one_page_first_color != false){
		$custom_css .='.main-navigation ul ul{';
			$custom_css .='border-bottom-color: '.esc_html($vw_one_page_first_color).';';
		$custom_css .='}';
	}
	...
	$theme_lay = get_theme_mod( 'vw_one_page_blog_layout_option','Default');
